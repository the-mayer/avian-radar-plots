auto.ARTI.history_v1.0 <- function(site.name,interval=3600,zoom=12,map.type="terrain"){
  ...
  url<-paste("http://ws.geonames.org/timezone?lat=",lat,"&lng=",lon,"&style=full","&username=herricks.group",sep="")
  u<-url(url,open="r")
  text<-readLines(u)
  local.time.zone <- text[grep("timezoneId",text)]
  local.time.zone <- sub("<timezoneId>","",tz)
  local.time.zone <- sub("</timezoneId>","",tz)
  close(u)
