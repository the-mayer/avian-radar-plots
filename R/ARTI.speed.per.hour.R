ARTI.speed.per.hour<-function(start.date.time,end.date.time,interval=3600,local.time.zone,title){
...
#Plot it!
  p<-ggplot(data=na.exclude(data),aes(x=Speed..mph))+geom_histogram(binwidth=1)
  final.graph<-p+xlab("Track Update Speed (mph)")+ggtitle(paste0(title,"\nTrack Update Speed (mph)"))+facet_wrap(~ breaks)+theme(plot.title = element_text(lineheight=.8, face="bold"))
  file.date<-as.character(as.POSIXct(start.date.time, format="%Y-%m-%d %H:%M:%S",tz=local.time.zone),format="%Y.%m.%d")
  filename<-paste0(file.date,"_Track.speed.per.hour.png")
