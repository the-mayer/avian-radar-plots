ARTI_Analysis_v2.3 <- function(radar.designation,track.history="Y",track.count="Y",track.heading="Y",track.speed="Y",track.altitude="Y",antenna.angle="Y",track.RCS="Y",track.range="Y",interval=3600,zoom=12,map.type="terrain"){
  
  #Add the spatial filtering and certain filtering rules
  ...
  if(length(filelist) >= 1){
    ...
    ##KML Classification Polygon  (use this section to filter the data by a polygon created in GE)
    ...
    # sub.data<-subset(data,is.na(Location.Classification)==F) #Only include data that is within a polygon.
    
    sub.data$Location.Classification<- NULL
    #sub.data$long.ID<- NULL
    
    data <- sub.data
    
    ## Define some rules for filter out the non-bird tracks
    
    sub.data <- subset(data, RCS..dBsm. >= -35 & RCS..dBsm. <= -10)
