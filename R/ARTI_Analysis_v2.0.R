ARTI_Analysis_v2.0 <- function(track.history="Y",track.count="Y",track.heading="Y",track.speed="Y",track.altitude="Y",antenna.angle="Y",interval=3600,zoom=12,map.type="terrain"){
  ...
  #Check if there are files to run... if not don't do anything
    if(length(filelist) >= 1){
      ...
      #Here is where we have radars!
        ...
        ifelse(round(radar.location$lon,digits=2) == round(-111.994217,digits=2)& round(radar.location$lat,digits = 2) == round(40.814600,digits=2),site.name<-"SLC - Accipiter Agile Antenna",print("Thinking"))
      ...
      #Break up data by week for really big datasets
          ...
          print("Creating Track Histories")
            ...
            #Graph it!
              for(x in 1:how.many.graphs){
                ...
                p1<-p  + geom_path(data=sub.data,aes(x=Longitude,y=Latitude,group=long.ID),color="dark red",size=.15,alpha=0.7, na.rm = T) #Plot the Track Lines
                ...
                ifelse(nrow(sub.data)>0,p9<-p8 + facet_wrap(~breaks,drop=T)+theme(axis.text=element_text(size=14,face="bold"),legend.position="none"),p9<-p8+theme(axis.text=element_text(size=14,face="bold"),legend.position="none")+ggtitle("No Data Recorded")) #Facet and add theme
              #Filename for the maps
                ...
                filename<-paste0(site.name,"_",file.time.string,"_",map.type,zoom,"_Track.History.jpeg")
              #Save it!
        ...
        if(track.count=="Y"){
          ...
          #p<-ggplot(data=na.exclude(sub.data),aes(x=breaks)) + geom_histogram(aes(fill=..count..),binwidth=24) + scale_fill_gradient("Count",low="green",high="red")+facet_wrap(~breaks2,scales="free_x",nrow=4) 
            p<-ggplot(sub.freqs, aes(x=names, y=x,fill=x)) + geom_bar(stat="identity") + scale_x_datetime(date_breaks="1 hour", labels=date_format("%H:%M",tz=local.time.zone))+ scale_fill_gradient2("Track Count",low="dark green",mid="yellow",high="dark red",limits=c(0,15000),midpoint=5000,space="Lab") + facet_wrap(~breaks2,scales="free_x",ncol=2) 
            intermediate.graph<-p+ xlab(paste0("Local Time (",local.time.zone,")"))+ylab("Frequency")+ggtitle(paste0(site.name,"\nTracks Per ",interval," seconds"))+ theme_bw() + theme(plot.title = element_text(lineheight=.8, face="bold")) + theme(axis.text.x=element_text(angle=45,hjust=1,size=6)) + theme(axis.text.y = element_text(size=6)) + theme(strip.text.x = element_text(size = 8))
        ...
        if(track.heading=="Y"){
          ...
          for(x in 1:length(unique(data$breaks2))){  
            ...
            #Save it!  
              ...
              no.data.filename<-paste0(file.date,"_NO.DATA_",site.name,"_Avg.Track.heading.per.",interval,".s.csv")
        ...
        if(track.speed=="Y"){
          ...
          for(x in 1:length(unique(data$breaks2))){
            ...
            #Save it!
              ...
              file.date<-as.character(unique(as.POSIXct(data$breaks2[x],local.time.zone)),format="%Y.%m.%d")
              no.data.filename<-paste0(file.date,"_NO.DATA_",site.name,"_Avg.Track.speed.per.",interval,"s.csv")
        ...
        if(track.altitude=="Y"){
          ...
          for(x in 1:length(unique(data$breaks2))){
            ...
            #Plot it!
              ...
              final.graph<-p+xlab("Average Track Altitude (ft)") + ylab("Frequency") + coord_flip()+ ggtitle(paste0(site.name,": Average Track Altitude (ft)","\nLocal Timezone: ", local.time.zone))+facet_wrap(~ breaks,ncol=6) + theme_bw() + theme(axis.text.x=element_text(angle=45,hjust=1,size=6)) + theme(plot.title = element_text(lineheight=.8, face="bold")) + theme(strip.text.x = element_text(size = 8)) + theme(axis.text.y = element_text(size=6))+ theme(panel.grid.major = element_line(size = .5, color = "grey"),axis.line = element_line(size=.7, color = "black"))
            #Save it!
              file.date<-as.character(unique(as.POSIXct(sub.data$breaks2,local.time.zone)),format="%Y.%m.%d")
              filename<-paste0(file.date,"_",site.name,"_Avg.Track.alt.per.",interval,"s.png")
              ...
              file.date<-as.character(unique(as.POSIXct(data$breaks2[x],local.time.zone)),format="%Y.%m.%d")
              no.data.filename<-paste0(file.date,"_NO.DATA_",site.name,"_Avg.Track.alt.per.",interval,"s.csv")
